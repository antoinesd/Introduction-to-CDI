...
TIP: *Decorator* are *bound* to a *bean type*
...
----
<1> Declares that the class is decorator
<2> Activates and give a priority for the decorator (could be done via `beans.xml` file)
<3> Decorator can be abstract and should share the same interface than decorated beans
