...
----
<1> `Instance<T>` injection point are always satisfied and never fails at deployment time
<2> with `Instance<T>` you control when bean instance is requested with the `get()` method
...
TIP: `AnnotationLiteral` is a helper class to ceate an annotation instance
...
----
<1> `select()` also accept a type.

TIP: you can use `TypeLiteral` to create instance of parameterized types