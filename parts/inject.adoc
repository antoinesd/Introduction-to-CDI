...
TIP: During boot time, the container scans each class to check if it meet conditions to be a bean
...
TIP: A bean can inject other beans or be injected in other beans
...
<2> The container look for the bean to inject by its _type_ (here `HelloService`)
...
----
<1> `@PostConstruct` is called after all injection are done
